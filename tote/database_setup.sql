...
# data input to tables from py script
SELECT * FROM races_data LIMIT 100;
SELECT COUNT(1) FROM races_data LIMIT 100;
SELECT COUNT(DISTINCT race_id) FROM races_data LIMIT 100;

SELECT * FROM horses_data LIMIT 100;
SELECT COUNT(1) FROM horses_data LIMIT 100;
SELECT COUNT(DISTINCT race_id) FROM horses_data LIMIT 100;

CREATE INDEX race_id ON races_data(race_id);
CREATE INDEX race_id ON horses_data(race_id);
CREATE INDEX horse_id ON horses_data(horse_id);


# explore data
SELECT COUNT(DISTINCT course), COUNT(DISTINCT surface), COUNT(DISTINCT going), COUNT(DISTINCT age), COUNT(DISTINCT distance), 
		COUNT(DISTINCT race_class), COUNT(DISTINCT runners), COUNT(DISTINCT stewards), COUNT(DISTINCT horse_name), COUNT(DISTINCT handicap) 
FROM horses_data;

SELECT race_date, pr_1_date, pr_2_date, pr_3_date, pr_4_date, pr_5_date, pr_6_date FROM horses_data WHERE race_date > '2018-01-01' LIMIT 1000;
# LOOKS LIKE PAST RESULTS ARE LATEST RESULTS REGARDLESS OF DATE


# check horses_data and races_data agree
SELECT hd.race_id, hd.course, mrd.course, hd.distance, mrd.distance,
		hd.going, mrd.going, hd.has_handicap, mrd.has_handicap
FROM (SELECT * FROM horses_data WHERE race_date BETWEEN '2019-06-01' AND '2019-12-01') hd
LEFT JOIN races_data mrd
ON hd.race_id = mrd.race_id
WHERE 1
AND hd.course <> mrd.course
AND hd.distance <> mrd.distance
AND hd.going <> mrd.going
AND hd.has_handicap <> mrd.has_handicap
LIMIT 100;

# check horses past results agree
SELECT hd1.pr_1_course_name, hd2.course, hd1.pr_1_date, hd1.race_date, hd2.race_date, hd1.pr_1_distance, hd2.distance,
		hd1.pr_1_going, hd2.going, hd1.pr_1_odds, hd2.betting_odds , hd1.pr_1_position, hd2.finish_position
FROM horses_data hd1
LEFT JOIN horses_data hd2
ON hd1.pr_1_race_id = hd2.race_id
AND hd1.horse_id = hd2.horse_id
LIMIT 100;

# add id and remove duplicates from horses_data
ALTER TABLE horses_data ADD id INT PRIMARY KEY AUTO_INCREMENT
;
CREATE TABLE horses_data_tmp AS SELECT * FROM horses_data
;
CREATE INDEX race_id ON horses_data_tmp(race_id)
;
CREATE INDEX horse_id ON horses_data_tmp(horse_id)
;
CREATE INDEX id ON horses_data_tmp(id)
;
SELECT COUNT(1) FROM (SELECT race_id, horse_id, COUNT(1) AS counts FROM horses_data GROUP BY race_id, horse_id) t WHERE counts>1
;
SELECT * FROM horses_data_tmp t1 INNER JOIN horses_data_tmp t2 ON t1.horse_id = t2.horse_id AND t1.race_id = t2.race_id AND t1.id < t2.id
;
DELETE FROM horses_data
WHERE id IN (SELECT t2.id FROM horses_data_tmp t1 INNER JOIN horses_data_tmp t2 ON t1.horse_id = t2.horse_id AND t1.race_id = t2.race_id AND t1.id < t2.id)
;
SELECT COUNT(1) FROM horses_data
;
DROP TABLE horses_data_tmp
;


# change finish distance to estimated yards, then use to create estimated finsh time
...
FROM horses_data
;
SELECT DISTINCT finish_distance FROM horses_times_pretmp ORDER BY finish_distance;
...
#DROP TABLE horses_times;
CREATE TABLE horses_times AS
...
CREATE INDEX race_id ON horses_times(race_id);
CREATE INDEX horse_id ON horses_times(horse_id);

DROP TABLE horses_times_tmp;
DROP TABLE horses_times_pretmp;

# create going categories for each race_id
#DROP TABLE races_goings;
CREATE TABLE races_goings AS
...
CREATE INDEX race_id ON races_goings(race_id);



# create table of handicaps in 
SELECT DISTINCT handicap FROM horses_data;
#DROP TABLE horses_handicaps;
CREATE TABLE horses_handicaps AS
...
CREATE INDEX race_id ON horses_handicaps(race_id);
CREATE INDEX horse_id ON horses_handicaps(horse_id);
...
#DROP TABLE races_types;
CREATE TABLE races_types AS
SELECT race_date, course, country, race_id, age, yards, name,
...
SELECT MIN(handicap_pounds), MAX(handicap_pounds), AVG(handicap_pounds)
FROM horses_handicaps
WHERE race_id IN (SELECT race_id FROM races_types WHERE race_type = 'chase' AND LOWER(country) IN ('England','Eire','Scotland','Wales'))
AND handicap_pounds <> 0
;

SELECT SUM(1) AS races,
		SUM(CASE WHEN yards <= 3300 THEN 1 ELSE 0 END) AS races_under_2miles,
        SUM(CASE WHEN yards <= 3520 THEN 1 ELSE 0 END) AS races_under_2miles,
		SUM(CASE WHEN yards > 3520 THEN 1 ELSE 0 END) AS races_over_2miles,
        SUM(CASE WHEN yards > 4820 THEN 1 ELSE 0 END) AS races_over_2p6miles
FROM races_data
WHERE race_id IN (SELECT race_id FROM races_types WHERE race_type = 'flat' AND LOWER(country) IN ('England','Eire','Scotland','Wales'))
;

SELECT rd.course, race_type, COUNT(1)
FROM races_data rd
LEFT JOIN races_types rt ON rd.race_id = rt.race_id
WHERE LOWER(rt.country) IN ('England','Eire','Scotland','Wales')
GROUP BY course, race_type
ORDER BY course
;

DROP TABLE races_types; # drop and recreate races_types with additional info
...
FROM races_data rd
LEFT JOIN (SELECT race_id, MIN(handicap_pounds) AS min_handicap_pounds, MAX(handicap_pounds) AS max_handicap_pounds FROM horses_handicaps GROUP BY race_id) hh
ON rd.race_id = hh.race_id
;

CREATE INDEX race_id ON races_types(race_id);

# create table based on past results to see if it sheds more light
#DROP TABLE races_pr_types;
CREATE TABLE races_pr_types AS
SELECT DISTINCT race_id, race_type
FROM
(
SELECT DISTINCT pr_1_race_id AS race_id, pr_1_run_type AS race_type FROM horses_data
UNION
SELECT DISTINCT pr_2_race_id AS race_id, pr_2_run_type AS race_type FROM horses_data
UNION
SELECT DISTINCT pr_3_race_id AS race_id, pr_3_run_type AS race_type FROM horses_data
UNION
SELECT DISTINCT pr_4_race_id AS race_id, pr_4_run_type AS race_type FROM horses_data
UNION
SELECT DISTINCT pr_5_race_id AS race_id, pr_5_run_type AS race_type FROM horses_data
UNION
SELECT DISTINCT pr_6_race_id AS race_id, pr_6_run_type AS race_type FROM horses_data
) t
;

CREATE INDEX race_id ON races_pr_types(race_id);

#DROP TABLE races_types_combined;
CREATE TABLE races_types_combined AS
...
FROM races_types rt
LEFT JOIN races_pr_types rpt ON rt.race_id = rpt.race_id
;

CREATE INDEX race_id ON races_types_combined(race_id);

SELECT race_type, COUNT(1)
FROM races_types_combined
WHERE 1
#AND race_type <> 'unknown'
AND lower(country) IN ('england','eire', 'northern ireland','wales','scotland')
GROUP BY race_type
LIMIT 3000;



# create table of race weathers
CREATE TABLE races_weather AS
SELECT race_id,
...
FROM races_data
;        
CREATE INDEX race_id ON races_weather(race_id);

# create big table of main features
SELECT * FROM horses_data LIMIT 100;
SELECT * FROM horses_times LIMIT 100;
SELECT * FROM horses_handicaps LIMIT 100;
SELECT * FROM races_data LIMIT 100;
SELECT * FROM races_goings LIMIT 100;
SELECT * FROM races_types_combined LIMIT 100;
SELECT * FROM races_weather LIMIT 100;
...
FROM horses_data hd
LEFT JOIN horses_times ht ON hd.race_id = ht.race_id AND hd.horse_id = ht.horse_id
LEFT JOIN horses_handicaps hh ON hd.race_id = hh.race_id AND hd.horse_id = hh.horse_id
LEFT JOIN races_data rd ON hd.race_id = rd.race_id
LEFT JOIN races_goings rg ON hd.race_id = rg.race_id
LEFT JOIN races_types_combined rtc ON hd.race_id = rtc.race_id
LEFT JOIN races_weather rw ON hd.race_id = rw.race_id
;

CREATE INDEX horse_id ON horses_data_combined_stg1(horse_id);
CREATE INDEX race_id ON horses_data_combined_stg1(race_id);

#DROP TABLE horses_data_combined;
CREATE TABLE horses_data_combined AS
SELECT hd.*, horse_race_number
FROM horses_data_combined_stg1 hd
LEFT JOIN (SELECT horse_id, race_id, ROW_NUMBER() OVER(PARTITION BY horse_id ORDER BY race_date, race_time) AS horse_race_number FROM horses_data) rn
ON hd.horse_id = rn.horse_id AND hd.race_id = rn.race_id
;

CREATE INDEX horse_id ON horses_data_combined(horse_id);
CREATE INDEX race_id ON horses_data_combined(race_id);
CREATE INDEX horse_race_number ON horses_data_combined(horse_race_number);

DROP TABLE horses_data_combined_stg1;



# create table with past 10 results
#DROP TABLE horses_data_combined_with_past_results;
CREATE TABLE horses_data_combined_with_past_results AS
SELECT 
...
CREATE INDEX horse_id ON horses_data_combined_with_past_results(horse_id);
CREATE INDEX race_id ON horses_data_combined_with_past_results(race_id);

SELECT COUNT(1) FROM horses_data_combined_with_past_results;
SELECT * FROM horses_data LIMIT 100;
SELECT * FROM horses_data_combined LIMIT 1000;
SELECT DISTINCT going_grouped FROM races_goings;





### Make similar table but based on no non-runners
CREATE TABLE horses_data_combined_no_non_runners_stg1 AS
SELECT * FROM horses_data_combined WHERE horse_time IS NOT NULL AND did_not_finish=0
;
...
LEFT JOIN horses_data t2 ON t1.race_id = t2.race_id AND t1.horse_id = t2.horse_id
;
CREATE TABLE horses_data_combined_no_non_runners AS
SELECT hd.*, horse_race_number
...
ON hd.horse_id = rn.horse_id AND hd.race_id = rn.race_id
;
CREATE INDEX horse_id ON horses_data_combined_no_non_runners(horse_id);
CREATE INDEX race_id ON horses_data_combined_no_non_runners(race_id);
CREATE INDEX horse_race_number ON horses_data_combined_no_non_runners(horse_race_number);


CREATE TABLE horses_data_combined_no_nrs_with_past_results AS
SELECT 
		# current race details
		h1.horse_id, h1.race_date, h1.race_id, h1.betting_odds, h1.finish_position, h1.horse_time, h1.did_not_finish, h1.finish_position_for_ordering,
        h1.course, h1.surface, h1.going, h1.yards, h1.runners, h1.prize1, h1.number_of_placed_rides,
...
FROM horses_data_combined_no_non_runners h1
...
CREATE INDEX horse_id ON horses_data_combined_no_nrs_with_past_results(horse_id);
