...
predictions_and_bets['Apreds'] = model1.predict(test_x_alt)[:,0]
predictions_and_bets['Dpreds'] = model1.predict(test_x_alt)[:,1]
predictions_and_bets['Hpreds'] = model1.predict(test_x_alt)[:,2]

# assign bets based on the probability being above a threshold higher than the bookies' probability
probability_margin = 0.00
probability_min = 0.5
...
print('away bets: ',sum(predictions_and_bets['Abets']), 'away winnings: ',sum(predictions_and_bets['Awinnings']))
print('draw bets: ',sum(predictions_and_bets['Dbets']), 'draw winnings: ',sum(predictions_and_bets['Dwinnings']))
print('home bets: ',sum(predictions_and_bets['Hbets']), 'home winnings: ',sum(predictions_and_bets['Hwinnings']))
